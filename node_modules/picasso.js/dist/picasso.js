  ...
  function resolve(_ref) {
    ...
    if (data && Array.isArray(data.items)) {
      for (var i = 0, len = data.items.length; i < len; i++) {
        var obj = deps.resolveForItem(data.items[i], cache.norm, data.items);
        obj.data = data.items[i];
        resolved.push(obj);
      }
    } else {
      var _obj = deps.resolveForItem(data, cache.norm);
      return {
        settings: cache.norm,
        item: _obj
      };
...
function buildShapes(_ref4) {
  var items = _ref4.items,
      ...
      height = _ref4.height,
      flipXY = _ref4.flipXY,
      majorStartRef = _ref4.majorStartRef,
      majorEndRef = _ref4.majorEndRef;
  ...
  var major = settings.major.scale;
  var minor = settings.minor.scale;

  for (var i = 0, len = items.length; i < len; i++) {
    var d = items[i].data;
    var children = [];

    var majorVal = items[i].major || items[i].majorStart;

    if (majorStartRef) {
      majorVal = major && d[majorStartRef] ? major(d[majorStartRef].value) : majorVal;
    }

    var majorEndVal = major && d[majorEndRef] ? major(d[majorEndRef].value) : null;
    ...
    var boxWidth = bandwidth * item.box.width;
...
function complexResolver(_ref) {
  ...
  for (var ki = 0, len = keys.length; ki < len; ki++) {
    ...
    for (var i = 0, extlen = ext[key].length; i < extlen; i++) {
...
var component = {
  ...
  render: function render(_ref) {
    ...
    var majorRef = this.settings.settings.major ? this.settings.settings.major.ref || '' : '';
    var majorStartRef = (typeof majorRef === 'undefined' ? 'undefined' : _typeof(majorRef)) === 'object' ? this.settings.settings.major.ref.start : null;
    var majorEndRef = (typeof majorRef === 'undefined' ? 'undefined' : _typeof(majorRef)) === 'object' ? this.settings.settings.major.ref.end : null;

    var shapes = buildShapes({ items: items, settings: settings, width: width, height: height, flipXY: flipXY, majorStartRef: majorStartRef, majorEndRef: majorEndRef });
